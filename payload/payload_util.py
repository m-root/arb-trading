from core.dex_calculator import DEXcalculator
...
class ArbitrageAggregator:
    ...
    def collectPriceData(self, exchange_name, spent_token, received_token, max_spent_amount, min_spent_amount,
                         digit_granularity, save_files=True):
        ...
        if self.exchangeBalances:
            ...
            spent_token_balance = self.DEX.fetchBalance(self.DEX.exchangeAddresses[exchange_name],
                                                        self.DEX.contractAddresses[spent_token])
            received_token_balance = self.DEX.fetchBalance(self.DEX.exchangeAddresses[exchange_name],
                                                           self.DEX.contractAddresses[received_token])
    ...
    def collectPriceDataOnAllExchanges(self, spent_token, received_token, max_spent_amount, min_spent_amount,
                                       digit_granularity, silent_mode=False):
        ...
        for exchange_name in self.exchangeBalances:
            price_dataframe = self.collectPriceData(exchange_name, spent_token, received_token, max_spent_amount,
                                                    min_spent_amount, digit_granularity, save_files=not (silent_mode))
            price_dataframe_dict[exchange_name] = price_dataframe
        return (price_dataframe_dict)

    def checkForArbitrage(self, token_A, token_B, max_spent_amount, min_spent_amount, digit_granularity,
                          silent_mode=False):
        ...
        token_A_data = self.collectPriceDataOnAllExchanges(token_A, token_B, max_spent_amount, min_spent_amount,
                                                           digit_granularity, silent_mode)
                ...
                if exchange_name_A != exchange_name_B:
                    ...
                    for index_A in token_A_data[exchange_name_A].index:
                        ...
                        token_B_sell_price = self.DEX.calculatePrice(self.exchangeBalances[exchange_name_B][token_B], \
                                                                     self.exchangeBalances[exchange_name_B][token_A],
                                                                     token_B_amount_bought)
                        ...
                        if total_profit > 0:
                            new_data_entry = {'Buy Exchange': exchange_name_A, \
                                              ...
                                              'Buy Price (' + token_A + ')':
                                                  token_A_data[exchange_name_A]['Price (' + token_A + ')'][index_A], \
                                              'Sell Price (' + token_A + ')': 1 / token_B_sell_price, \
                                              'Traded Amount (' + token_B + ')': token_B_amount_bought, \
                                              'Spent Amount (' + token_A + ')':
                                                  token_A_data[exchange_name_A]['Spent Amount'][index_A], \
                    ...
                    if not (silent_mode):
                        print('    ' + str(
                            len(arbitrage_data_list) - number_of_arbitrage_opportunities_found) + ' arbitrage opportunities found!')
    ...
    def loopArbitrageDetection(self, token_A, token_B, max_spent_amount, min_spent_amount, digit_granularity):
        ...
        while collect_data:
            try:
                arbitrage_dataframe = self.checkForArbitrage(token_A, token_B, max_spent_amount, min_spent_amount,
                                                             digit_granularity, silent_mode=True)
                ...
                for buy_exchange in self.exchangeBalances:
                    for sell_exchange in self.exchangeBalances:
                        if buy_exchange != sell_exchange:
                            ...
                            exchange_pair_dataframe = \
                            arbitrage_dataframe[arbitrage_dataframe['Buy Exchange'] == buy_exchange] \
                                [arbitrage_dataframe['Sell Exchange'] == sell_exchange]
                            new_statistics_entry.update(
                                {exchange_pair + ' # of Profitable Trades': len(exchange_pair_dataframe), \
                                 exchange_pair + ' Total Profit': sum(
                                     exchange_pair_dataframe['Trade Profit (' + token_A + ')']), \
                                 exchange_pair + ' Biggest Trade Profit': exchange_pair_dataframe[
                                     'Trade Profit (' + token_A + ')'].max(), \
                                 exchange_pair + ' Average Profit': exchange_pair_dataframe[
                                     'Trade Profit (' + token_A + ')'].mean(), \
                                 exchange_pair + ' Median Profit': exchange_pair_dataframe[
                                     'Trade Profit (' + token_A + ')'].median()})
    ...
    def saveDataToCSV(self, data, file_name):
        ...
        full_file_name = file_name + '_' + str(date_time[1]) + '-' + str(date_time[2]) + '-' + str(date_time[0]) + '_' + \
                         '0' * (2 - len(str(date_time[3]))) + str(date_time[3]) + '-' + '0' * (
                                     2 - len(str(date_time[4]))) + \
